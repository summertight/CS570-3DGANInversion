...
@click.option('--loss_choice', help='Select Losses', type=click.Choice(['l1', 'vgg', 'id','multi_depth','multi_shape','depth']), default=['l1','vgg'],multiple=True)
...
def generate_images(
    ...
    #import pdb;pdb.set_trace()
    predefined_names = ['minho.png','jaeseong2.png','xxx.png','dua.png','kanye.png','candal.png','drake.png']
    ...
    for idx, datum in enumerate(path_imgs):
        ...
        with open(os.path.join(data_path.replace('inthewild_data','wild_w_picked_pickles'), datum+'.pickle'), 'rb') as f:
            w_pickle = pickle.load(f)

        w = w_pickle['ws'].repeat(1,14,1).to(device)
        depth_gts=[]
        #shape_gts=[]
        if 'depth' in loss_choice:
            depth_gt = G.synthesis(w, c=c)['image_depth']
        w_opt = G.backbone.mapping.w_avg.unsqueeze(0).unsqueeze(1).repeat(1,14,1).detach().cpu().cuda()
        ...
        for angle_y, angle_p in [(.4, angle_p), (0, angle_p), (-.4, angle_p)]:
            ...
            depth_gt_temp = G.synthesis(w ,None, camera_params)['image_depth']
            depth_gts.append(depth_gt_temp)
        ...
        for step in range(steps):
            ...
            if 'multi_depth' in loss_choice or 'multi_shape' in loss_choice:
                angle_p = -0.2
                ...
                for idx, (angle_y, angle_p) in enumerate([(.4, angle_p), (0, angle_p), (-.4, angle_p)]):
                    ...
                    if 'multi_depth' in loss_choice:    
                        if idx ==0:
                            loss_multi_depth_right = torch.nn.functional.l1_loss(depth_gts[idx], result_temp['image_depth'])
                            loss += .1*loss_multi_depth_right
                        if idx ==1:
                            loss_multi_depth_middle = torch.nn.functional.l1_loss(depth_gts[idx], result_temp['image_depth'])
                            loss += .1*loss_multi_depth_middle
                        if idx ==2:
                            loss_multi_depth_left = torch.nn.functional.l1_loss(depth_gts[idx], result_temp['image_depth'])
                            loss += .1*loss_multi_depth_left
                    if 'multi_shape' in loss_choice:
                        
                        image_side_cropped = landmark_detector.align_face(
                            inputs=denorm(img_temp), scale=1.25, inverse=False, target_size=224)
                    
                    
                        codedict_gen = criterion_DECA.encode(image_side_cropped,use_detail=False)#XXX (0,1)

                        image_gt_cropped = landmark_detector.align_face(
                            inputs=denorm(img), scale=1.25, inverse=False, target_size=224)

                        codedict_gt = criterion_DECA.encode(image_gt_cropped,use_detail=False)

                        loss_3dmm_shape = torch.nn.functional.l1_loss(codedict_gen['shape'], codedict_gt['shape'])
                        loss += loss_3dmm_shape
            ...
            if 'depth' in loss_choice:
                loss_depth = torch.nn.functional.l1_loss(depth_gt, result['image_depth'])
                loss += .1*loss_depth
            if 'vgg' in loss_choice:
                #import pdb;pdb.set_trace()
            ...
            if 'id' in loss_choice:
                ...
                if 'full' in loss_to:
                    breakpoint()
                    _,_,ldmk_img = mtcnn.detect((img_mini.type(torch.float64).permute(0,2,3,1)+1)/2*255,landmarks=True)
                    _,_,ldmk_img_rec = mtcnn.detect((img_rec_mini.type(torch.float64).permute(0,2,3,1)+1)/2*255,landmarks=True)
                    img_aligned = align_and_crop_with_5points_tensor(img_mini, torch.tensor(ldmk_img,device=device).squeeze(1))
                    img_rec_aligned = align_and_crop_with_5points_tensor(img_mini, torch.tensor(ldmk_img_rec,device=device).squeeze(1))
                    
                    loss_id = criterion_ID(img_aligned, img_rec_aligned).mean()
                    loss = loss + loss_id
                if 'raw' in loss_to:
            ...
            if step%100==0:
                ...
                if 'depth' in loss_choice:
                    print(f'Step: {step}/Loss(Depth):{loss_depth}')
                if 'multi_depth' in loss_choice:
                    print(f'Step: {step}/Loss(Depth_R):{loss_multi_depth_right}')
                    print(f'Step: {step}/Loss(Depth_M):{loss_multi_depth_middle}')
                    print(f'Step: {step}/Loss(Depth_L):{loss_multi_depth_left}')
                if 'multi_shape' in loss_choice:
                    print(f'Step: {step}/Loss(Multi_DECA_shape):{loss_3dmm_shape}')
