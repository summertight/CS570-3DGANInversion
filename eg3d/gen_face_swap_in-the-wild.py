...
def generate_images(
    ...
    ldmk_tgt = np.load(os.path.join(data_path.replace('_data','_data_kps'),tgt_name+'.npy'),allow_pickle=True)
    ...
    if True:
        ...
        cam_tgt = label[tgt_name]
        c_tgt = torch.from_numpy(np.array(cam_tgt)).to(torch.float32).to(device).unsqueeze(0)
        ...
        img_tgt = PIL.Image.open(img_tgt_path).resize((512,512), PIL.Image.LANCZOS)
        ...
        with open(os.path.join(data_path.replace('inthewild_data','wild_w+_picked_pickles'), src_name+'.pickle'), 'rb') as f:
            w_pickle_src = pickle.load(f)
        with open(os.path.join(data_path.replace('inthewild_data','wild_w+_picked_pickles'), tgt_name+'.pickle'), 'rb') as f:
            w_pickle_tgt = pickle.load(f)
        w_src = w_pickle_src['ws'].to(device)
        w_tgt = w_pickle_tgt['ws'].to(device)

        with open(os.path.join(data_path.replace('inthewild_data','wild_w_picked_pickles'), src_name+'.pickle'), 'rb') as f:
            w_pickle_src_single = pickle.load(f)
        with open(os.path.join(data_path.replace('inthewild_data','wild_w_picked_pickles'), tgt_name+'.pickle'), 'rb') as f:
            w_pickle_tgt_single = pickle.load(f)
        w_src_single = w_pickle_src_single['ws'].to(device)
        w_tgt_single = w_pickle_tgt_single['ws'].to(device)
        ...
        global_name = f"{src_name}_{tgt_name}_{str(loss_choice)}"
        ...
        for step in range(steps):
            ...
            result_tgt, feat_list_tgt = G.synthesis_extract_feat(w_tgt, c=c_tgt)
            result_tgt_single, feat_list_tgt_single \
                = G.synthesis_extract_feat(w_tgt_single.repeat(1,14,1), c=c_tgt)
            ...
            img_depth_w = result_tgt_single['image_depth']
            ...
            if 'depth_reg' in loss_choice:
                loss_depth = torch.nn.functional.l1_loss(img_depth_w,img_depth_rec).mean()
                loss +=loss_choice['depth_reg']*(loss_depth)

                loss_logger['depth_reg'] = loss_depth
            ...
            if 'l1_mask' in loss_choice:
                
                
                msk_tgt[msk_tgt == 0] = 0
                loss_l1_mask = torch.nn.functional.l1_loss(img_rec*msk_tgt, img_tgt*msk_tgt).mean()
                loss+= loss_choice['l1_mask'] * loss_l1_mask

                loss_logger['l1_mask'] = loss_l1_mask
            ...
            if 'id_contra' in loss_choice:
                loss_id_contra = criterion_ID.forward_contrastive(img_src_aligned, img_tgt_aligned, img_rec_aligned).mean()

                loss += loss_choice['id_contra']* loss_id_contra

                loss_logger['id_contra'] = loss_id_contra

            if 'feat_16' in loss_choice:
                loss_feat16 = torch.nn.functional.l1_loss(feat_list_tgt[2], feat_list[2])
                loss += loss_choice['feat_16']* loss_feat16

                loss_logger['feat_16'] = loss_feat16

            if 'feat_32' in loss_choice:
                loss_feat32 = torch.nn.functional.l1_loss(feat_list_tgt[3], feat_list[3])
                loss += loss_choice['feat_32']* loss_feat32

                loss_logger['feat_32'] = loss_feat32

            if 'feat_64' in loss_choice:
                loss_feat64 = torch.nn.functional.l1_loss(feat_list_tgt[4], feat_list[4])
                loss += loss_choice['feat_64']* loss_feat64

                loss_logger['feat_64'] = loss_feat64
            
            if 'deca_shape' in loss_choice:
                image_src_cropped = landmark_detector.align_face(
                    inputs=denorm(img_src), scale=1.25, inverse=False, target_size=224)
            

                codedict_src= criterion_DECA.encode(image_src_cropped,use_detail=False)#XXX (0,1)
                
                image_rec_cropped = landmark_detector.align_face(
                    inputs=denorm(img_rec), scale=1.25, inverse=False, target_size=224)

                codedict_rec = criterion_DECA.encode(image_rec_cropped,use_detail=False)

                loss_3dmm_shape = torch.nn.functional.l1_loss(codedict_src['shape'], codedict_rec['shape'])
                loss += loss_choice['deca_shape']*loss_3dmm_shape

                loss_logger['deca_shape'] = loss_3dmm_shape
     
            if 'deca_light' in loss_choice:
                image_tgt_cropped = landmark_detector.align_face(
                    inputs=denorm(img_tgt), scale=1.25, inverse=False, target_size=224)

                codedict_tgt = criterion_DECA.encode(image_tgt_cropped,use_detail=False)

          
                loss_3dmm_light = torch.nn.functional.l1_loss(codedict_tgt['light'], codedict_rec['light'])
                loss += loss_choice['deca_light']*loss_3dmm_light

                loss_logger['deca_light'] = loss_3dmm_light
            ...
            if step%100==0:
                ...
                img_rec_masked_save = ((img_rec*msk_tgt).permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
               
                imgs_save.append(img_src_save)
                imgs_save.append(img_tgt_save)
                imgs_save.append(img_rec_save)
                imgs_save.append(img_rec_masked_save)
                angle_p = -0.2

                depths_save = []
                #img = (img - lo) * (255 / (hi - lo))
                depth_w_save = ((img_depth_w.permute(0, 2, 3, 1)-img_depth_w.min()) * 255/(img_depth_w.max()-img_depth_w.min())).to(torch.uint8)
                depth_rec_save = ((img_depth_rec.permute(0, 2, 3, 1)-img_depth_rec.min()) * 255/(img_depth_rec.max()-img_depth_rec.min())).to(torch.uint8)
                
                depths_save.append(depth_w_save)
                depths_save.append(depth_rec_save)
