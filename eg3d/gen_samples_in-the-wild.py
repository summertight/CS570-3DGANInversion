...
@click.command()
@click.option('--network', 'network_pkl', help='Network pickle filename', required=False, default='/home/nas4_user/jaeseonglee/ICCV2023/eg3d_ckpts/eg3d-fixed-triplanes-ffhq.pkl')
...
def generate_images(
    ...
    with open(os.path.join(data_path, 'out_kanye_depth_pickles', datum+'.pickle'), 'rb') as f:
        w_depth_pickle = pickle.load(f)
    w_depth = w_depth_pickle['ws'].to(device)
    #with open(os.path.join(data_path, 'out_candal_depth_multi_pickles', datum+'.pickle'), 'rb') as f:
    #    w_depth_multi_pickle = pickle.load(f)
    #w_depth_multi = w_depth_multi_pickle['ws'].to(device)
    w_bunch = {'w':w,'wp':wp,'w_depth':w_depth,}#'w_depth_multi':w_depth_multi}
    for ws_key in w_bunch.keys():
        ws = w_bunch[ws_key]
        imgs = []
        angle_p = -0.2
        ...
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/seed{ws_key}.png')
        ...
        if shapes:
            ...
            with tqdm(total = samples.shape[1]) as pbar:
                with torch.no_grad():
                    while head < samples.shape[1]:
                        torch.manual_seed(0)
                        sigma = G.sample_from_ws(samples[:, head:head+max_batch], transformed_ray_directions_expanded[:, :samples.shape[1]-head], ws, conditioning_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, noise_mode='const')['sigma']
            ...
            if shape_format == '.ply':
                ...
                with mrcfile.new_mmap(os.path.join(outdir, f'seed{ws_key}.mrc'), overwrite=True, shape=sigmas.shape, mrc_mode=2) as mrc:
                    mrc.data[:] = sigmas
