...
def init_dataset_kwargs(data, mode, n_of_data):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data, use_labels=True, max_size=None, xflip=False, mode=mode, n_of_data=n_of_data)
...
@click.option('--data',         help='Training data', metavar='[ZIP|DIR]',                      type=str, default='/home/nas4_user/jaeseonglee/4D-Facial-Avatars/nerface_dataset/person_2/train/crop')
...
@click.option('--neural_rendering_resolution_initial', help='Resolution to render at', metavar='INT',  type=click.IntRange(min=1), default=128, required=False)
...
@click.option('--mode', help='autoencoder or eg3d', metavar='STR',  type=click.Choice(['single_id','multi_id', 'opt']), required=True, default='single_id')
@click.option('--pretrain', help='autoencoder or eg3d', metavar='STR',  type=str,  default='/home/nas4_user/jaeseonglee/ICCV2023/eg3d_ckpts/eg3d-fixed-triplanes-ffhq.pkl')
...
#XXX 클릭 옵션은 소문자만 취급
@click.option('--n_of_data', default= -1, type = int)
...
def main(**kwargs):
    ...
    if opts.mode == 'single_id' or 'opt':
        c.mode = opts.mode
        c.E_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
        c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.InversionSingleIDLoss')

    #c.id_path = opts.id_path

        
    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, mode=opts.mode, n_of_data=opts.n_of_data)
