...
@click.option('--swap_type', help='Swap type map or w', type=click.Choice(['map', 'w', 'w_bunch']), default='w_bunch')
#@click.option('--swap_type', help='Swap type map or w', type=click.Choice(['map', 'w', 'w_bunch']), default='w')
...
def generate_images(
    ...
    os.makedirs(os.path.join('/'.join(data_path.split('/')[:-1]), outdir),exist_ok=True)
    ...
    for datum in data:
        ...
        with open(os.path.join('inthewild_data', 'dataset.json')) as f:
            label = json.load(f)['labels']
        
        with open(os.path.join(data_path,datum[:-7]+'.pickle'), 'rb') as f:
            data1 = pickle.load(f)
        datum2 = random.choice(data)
        with open(os.path.join(data_path,datum2[:-7]+'.pickle'), 'rb') as g:
            data2 = pickle.load(g)
        label = dict(label)
        #cam = label[datum]
        ...
        if swap_type == 'map':
            ...
            for i in range(len(maps1)):
                ...
                PIL.Image.fromarray(result[0].detach().cpu().numpy(), 'RGB').save(os.path.join('/'.join(data_path.split('/')[:-1]), outdir)+f'/{datum}_and_{datum2}_{i}_swapped.png')
            ...
            for i in range(ws1.shape[1]):
                ...
                temp_ws2 = ws2.clone()
                #import pdb;pdb.set_trace()
                temp_ws1[0][i] = ws2[0][i].clone()
                temp_ws2[0][i] = ws1[0][i].clone()
                ...
                PIL.Image.fromarray(result[0].detach().cpu().numpy(), 'RGB').save(os.path.join('/'.join(data_path.split('/')[:-1]), outdir)+f'/{datum.split(".")[0]}_and_{datum2.split(".")[0]}_{i}_swapped.png')
            ...
            for i in range(3):
                ...
                if i == 0:
                    nm  = 'coarse'
                    temp_ws1[0][:4] = ws2.clone()[0][:4]
                    temp_ws2[0][:4] = ws1.clone()[0][:4]

                elif i==1:
                    nm = 'middle'
                    temp_ws1[0][4:8] = ws2.clone()[0][4:8]
                    temp_ws2[0][4:8] = ws1.clone()[0][4:8]

                elif i==2:
                    nm = 'fine'
                    temp_ws1[0][8:] = ws2.clone()[0][8:]
                    temp_ws2[0][8:] = ws1.clone()[0][8:]
                ...
                PIL.Image.fromarray(result[0].detach().cpu().numpy(), 'RGB').save(os.path.join('/'.join(data_path.split('/')[:-1]), outdir)+f'/{datum.split(".")[0]}_and_{datum2.split(".")[0]}_{nm}_swapped.png')
