...
class StyleGAN2AELoss(Loss):
    ...
    def run_EG_straight(self, img_src, c, neural_rendering_resolution):
        ...
        if self.invert_map:
            ...
            ws = self.E.encode(img_src_resized)
            gen_output = self.G.synthesis(ws, None, c, neural_rendering_resolution=neural_rendering_resolution)
            ...
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if 'deca' in self.loss_selection:
                    ...
                    if ldmks_gen.shape[-1] == 0:
                        ...
                        ldmks_gt = self.FAN.run((real_img['image_src']+1)*127.5)#XXX (0,255)
                        image_cropped_gt, _ = self.cropper.crop(image = real_img['image_src'],points = torch.tensor(ldmks_gt))
                        codedict_gt = self.criterion_DECA.encode((image_cropped_gt + 1)/2)#XXX (0,1)
                        #import pdb;pdb.set_trace()
                        loss_3dmm_shape = torch.nn.functional.mse_loss(codedict_gen['shape'], codedict_gt['shape'])
                        loss_3dmm_exp = torch.nn.functional.mse_loss(codedict_gen['exp'], codedict_gt['exp'])
