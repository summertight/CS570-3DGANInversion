...
def align_and_crop_with_5points_tensor(img, landmark, size=224):
        ...
        REFERENCE_FACIAL_POINTS = torch.tensor(REFERENCE_FACIAL_POINTS, dtype=torch.float64, device=landmark.device)
        ...
        tfm = torch.tensor([[1, 0, 0], [0, 1, 0]],device=landmark.device).unsqueeze(0).repeat(B,1,1)
        n_pts = dst_pts.shape[1]
        ones = torch.ones((B, n_pts, 1), dtype=torch.float64,device=landmark.device)
        ...
        try:
                #tfm = torch.tensor([[A[..., 0, 0], A[..., 1, 0], A[..., 2, 0]],[A[..., 0, 1], A[..., 1, 1], A[..., 2, 1]]],dtype=torch.float32,device=landmark.device).unsqueeze(0)
                tfm = A[:,:,:2].permute(0,2,1).type(torch.float32)
        except:
