...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            _, _, acc2, std2, xnorm, _ = verification.test(
                pair_list=self.ver_list[i][0], 
                label_list=self.ver_list[i][1], 
                net=backbone, nfolds=10)
    ...
    def init_dataset(self, val_targets, data_dir, image_size):
        for name in val_targets:
            ...
            name = name + '.txt'
            if os.path.exists(os.path.join(txt_root, name)):
                # data_set = verification.load_bin(path, image_size)
                dataset = verification.control_text_list(txt_root=txt_root, txt_dir=name)
                self.ver_list.append(dataset)
