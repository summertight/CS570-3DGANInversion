...
class PartialFC(torch.nn.Module):
    ...
    def __init__(
        ...
        if inter_prototype_file != '':
            self.child_identity_activated = []
            # print(f'age files from {inter_prototype_file}')
            # age_file = open(inter_prototype_file).readlines()
            # self.id2age = {os.path.join(str(int(line.split(' ')[1].split('/')[1])), str(int(line.split(' ')[1].split('/')[2][:-4]))) : float(line.split(' ')[2]) for line in age_file}
            # self.child_image2age = { os.path.join(str(int(line.split(' ')[1].split('/')[1])), str(int(line.split(' ')[1].split('/')[2][:-4]))) : float(line.split(' ')[2]) for line in age_file if float(line.split(' ')[2]) < 13}
            ...
            # self.child_identity_freq = {int(k): v for k, v in sorted(self.child_image2freq.items(), key=lambda item: item[1]) if v >= child_filter}
            # self.child_identity = list(self.child_identity_freq.keys())

            with open(f'{inter_prototype_file}', 'rb') as f:
                self.child_identity = pickle.load(f)
            self.child_identity = np.intersect1d(np.array(self.child_identity), np.arange(self.class_start, self.num_local + self.class_start))
            self.child_identity = self.child_identity - self.class_start
            self.child_identity = list(self.child_identity)
            print(f'num local:{self.num_local}')

            self.criterion_prototype = torch.nn.MSELoss()
    ...
    def sample(self, 
        ...
        self.child_identity_activated = np.intersect1d(np.array(self.child_identity), self.weight_index.detach().cpu().numpy())
        self.child_identity_activated = list(self.child_identity_activated)
        self.weight_inter_activated = torch.nn.Parameter(self.weight[self.child_identity_activated])
    ...
    def forward(
        ...
        # print(f'list embeddings before: {len(_list_embeddings)}')
        # print(f'gather labels before: {len(_gather_labels)}')

        embeddings = torch.cat(_list_embeddings)
        labels = torch.cat(_gather_labels)

        # print(f'embeddings after: {embeddings}')
        ...
        # print(f'index_positive: {index_positive}')
        ...
        if self.child_identity is not None:
            #norm_weight_activated 가 prototype에서 해당하는 index를 구해야함
            # kernel = norm_weight_activated[self.child_identity_activated, :]
            with torch.cuda.amp.autocast(self.fp16):
                kernel = normalize(self.weight_inter_activated)
            # print(self.rank)
            # print(norm_weight_activated.shape)
            # print(max(self.child_identity_activated))
            # print()
            # while True:
            #     pass
            prototype_matrix = torch.mm(kernel, kernel.T)
            child_loss = self.criterion_prototype(prototype_matrix, torch.eye(prototype_matrix.shape[0]).to(prototype_matrix.device))
            # child_loss = 0.
            loss = self.dist_cross_entropy(logits, labels) + child_loss
        else:
